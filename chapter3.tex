...
\State initialize first stage simulation input vector $altIds \gets \emptyset$ \Comment{use id of alternative to represent the simulation input}
\For{$i = 0 \to k - 1$}
  \For{$j = 0 \to n_0 - 1$}
    \State append $i$ into $altIds$
  \EndFor
\EndFor
\State simulate against all simulation input in $altIds$ \textbf{in parallel}, and store the result into $X_{ij}$ \\
\State initialize first stage simulation input vector $altIds \gets \emptyset$
\For{$i = 0 \to k - 1$}
  ...
  \EndFor
\EndFor
\State simulate against all simulation input in $altIds$ \textbf{in parallel}, and store the result into $X_{ij}$ \Comment{this time $j \geq n_0$} \\
\For{$i = 0 \to k - 1$}
  \State $\bar{X_i}(N_0) = \frac{1}{N_0} \sum_{j=0}^{N_0 - 1}X_{ij}$
\EndFor \\
\Return $\argmax_{i}\bar{X_i}(N_0)$
\end{algorithmic}
...
\Require $k, \alpha, \delta, n_0$
\State $h \gets$ the Rinott's const with arguments $k, \alpha, n_0$ \\
\State initialize simulation input vector $altIds \gets \emptyset$ \Comment{use id of alternative to represent the simulation input}
\For{$i = 0 \to k - 1$}
  \For{$j = 0 \to n_0 - 1$}
    \State append $i$ into $altIds$
  \EndFor
\EndFor
\State start simulating against all simulation input in $altIds$ \textbf{in parallel and asynchronously}
\State $samplingCount \gets k \times n_0$ \\
\State initialize $count[k], sum[k], sumOfSquare[k]$
\While {$samplingCount > 0$}
  \State $simOutput \gets $ take one generated simulation output
  \State $i \gets simOutput.altId$
  \State update corresponding $count[i], sum[i], sumOfSquare[i]$
  \State $samplingCount \gets samplingCount -1$
  \If{$count[i] = n_0$}
	\State $mean \gets sum[i] / count[i]$
	\State $S2 \gets (sumOfSquare[i] - count[i] \times mean \times mean) / (count[i] - 1)$
	\State $N \gets \max\{n_0, \lceil \frac{h^2S2}{\delta^2} \rceil\}$
	\If{$N > n_0$}
	  \State initialize simulation input vector $altIds \gets \emptyset$
      \For{$j = 0 \to (N - n_0) - 1$}
        \State append $i$ into $altIds$
      \EndFor
	  \State add simulations against input in $altIds$ as new \textbf{asynchronous} task
	  \State $samplingCount \gets samplingCount + (N - n_0)$
	\EndIf
  \EndIf
\EndWhile\\
\\
\Return $\argmax_{i} sum[i] / count[i]$
\end{algorithmic}
